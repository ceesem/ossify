...
def _bfs_shortest_path(
    ...
    """Find shortest path between two nodes in an adjacency dict using BFS.

    Parameters
    ----------
    adjacency :
        Dict mapping node names to sets of neighbor names.
    source :
        Starting node.
    target :
        Destination node.
    ...
    -------
    :
        List of node names from source to target (inclusive).
...
class MorphSync:
    ...
    @property
    def _link_adjacency(self) -> dict[str, set[str]]:
        """Adjacency dict representing connectivity between layers.

        Returns
        -------
        :
            Dict mapping layer names to sets of connected layer names.
        """
    ...
    def get_link_path(self, source: str, target: str) -> list[str]:
        ...
        """
        return _bfs_shortest_path(self._link_adjacency, source, target)
